...
void InitDaemon(bool daemonize)
{
    ...
    signal(SIGINT, [](int) { closingRequested = true; });

    signal(SIGTERM, [](int) { closingRequested = true; });
...
{
  public:
    Options(int argc, const char *const argv[]) : valid_(false)
    {
        // Options that this application recognizes.
        known_ = {"no-daemon", "debug", "help"};
...
int main(int argc, char const *argv[])
{
    ...
    if (options.has("no-daemon"))
    {
        daemonize = false;
        sLog.ToConsole(
            "Running without daemonization, logs will be written in console");
    ...
    {
        sLog.log(LOG_FLAG_ERROR, SERVER_DAEMON_NAME + " failed to init SSL: " +
                                     std::string(e.what()));
    ...
    catch (CommunoException &e)
    {
        sLog.log(LOG_FLAG_ERROR,
                 SERVER_DAEMON_NAME +
                     " failed to init socket: " + std::string(e.what()));
        e.SendSystemErrDialogBox();
    }
    ...
    try
    {
        sLog.log(LOG_FLAG_DEBUG,
                 SERVER_DAEMON_NAME + " InitEntities launched.");
        server.InitEntities();
    ...
    catch (CommunoException &e)
    {
        sLog.log(LOG_FLAG_ERROR,
                 SERVER_DAEMON_NAME +
                     " failed to init InitEntities: " + std::string(e.what()));
        e.SendSystemErrDialogBox();
    }
    ...
    while (server.getServerState() != eServerState::CLOSED)
    {
        ...
        {
            sLog.log(LOG_FLAG_ERROR, SERVER_DAEMON_NAME + " runtime error: " +
                                         std::string(e.what()));
            e.SendSystemErrDialogBox();
        }
        ...
        if (closingRequested)
        {
            sLog.log(LOG_FLAG_INFO,
                     SERVER_DAEMON_NAME + " closing requested...");
        ...
        if (server.getServerState() == eServerState::CLOSING)
        {
            server.CloseServer();
            sLog.log(LOG_FLAG_INFO,
                     SERVER_DAEMON_NAME + "server is now closed.");
        }
        else
        {
            usleep(50);
