...
{
    LOG_LEVEL_DEBUG =
        LOG_FLAG_DEBUG | LOG_FLAG_INFO | LOG_FLAG_WARN | LOG_FLAG_ERROR,
...
class Logger
{
  public:
    Logger()
        : m_LogLevelFilter(LOG_LEVEL_INFO), m_LogOutputFilter(LOG_CONSOLE),
          m_LogOutputFile("")
    {
    }

    void ToConsole(const char *msg) { ToConsole(std::string(msg)); }

    void ToConsole(const std::string &msg) { std::cout << msg << std::endl; }
    ...
    void setLogOutput(eLoggingOutputFlags outFlags)
    {
        m_LogOutputFilter = outFlags;
    }
